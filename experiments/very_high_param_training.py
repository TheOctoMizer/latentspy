...
def run_experiment():
    EXPERIMENT_NAME: str = "unstable_high_lr_training"
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "mps" if torch.mps.is_available() else "cpu")
    print(f"Using device: {DEVICE}")

    # UNSTABLE HYPERPARAMETERS (The point of comparison)
    LEARNING_RATE = 0.1
    WARMUP_STEPS = 0
    MAX_GRAD_NORM = 100.0   # Essentially no clipping

    # OBSERVATION CONFIG (Kept identical to baseline for direct comparison)
    BATCH_SIZE = 16
    NUM_EPOCHS = 3
    VAL_INTERVAL = 750
    SAMPLE_INTERVAL = 200
